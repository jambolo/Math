...
{
	if (    Math::IsRelativelyCloseTo( a.m_X, b.m_X )
		 && Math::IsRelativelyCloseTo( a.m_Y, b.m_Y )
		 && Math::IsRelativelyCloseTo( a.m_Z, b.m_Z ) )
	{
		return INTERSECTS;
	}
	else
...
//
//	return Math::IsCloseToZero( line.Distance( point ) );
//
	// Yes, but also ...
	...
	if ( Math::IsRelativelyCloseTo( d * d, pb.Length2() ) )
	{
		return INTERSECTS;
	}
	else
...
//	// The point is on the line if the distance is close to 0
//
//	return Math::IsCloseToZero( ray.Distance( point ) );
//
	...
	if ( t >= 0.f && Math::IsRelativelyCloseTo( t * t, pb.Length2() ) )
	{
		return INTERSECTS;
	}
	else
...
//
//	return Math::IsCloseToZero( segment.Distance( point ) );
//
	// Yes, but also like the line ...
	// point is on the ray if the distance from m_B to point is the same as the distance from m_B to the projection
	...
	if ( a >= 0.f && a <= s2 && Math::IsRelativelyCloseTo( a * a, pb.Length2() ) )
	{
		return INTERSECTS;
	}
	else
...
{
	if ( Math::IsCloseToZero( Distance( point, plane ) ) )
	{
		return INTERSECTS;
	}
	else
...
{
	if ( Math::IsCloseToZero( Distance( a, b ) ) )
	{
		return INTERSECTS;
	}
	else
...
{
	if ( !Math::IsCloseToZero( Dot( line.m_M, plane.m_N  ) ) )
	{
		return INTERSECTS;
	}
	else
...
Intersectable::IntersectionClass Intersectable::Intersects( Line const & line, AABox const & aabox )
{
	...
	if ( !Math::IsCloseToZero( line.m_M.m_X ) )
	{
	...
	if ( !Math::IsCloseToZero( line.m_M.m_Y ) )
	{
	...
	if ( !Math::IsCloseToZero( line.m_M.m_Z ) )
	{
...
Intersectable::IntersectionClass Intersectable::Intersects( Segment const & segment, Plane const & plane )
{
	...
	if ( Math::IsCloseToZero( da ) || Math::IsCloseToZero( db ) || da * db < 0.f )
	{
		return INTERSECTS;
	}
	else
...
Intersectable::IntersectionClass Intersectable::Intersects( Plane const & a, Plane const & b )
{
	...
	if ( !Math::IsCloseTo( fabsf( dot ), 1.0 ) )
	{
		return INTERSECTS;
	}
	else
...
//	if ( !Math::IsCloseToZero( plane.m_N.m_X )
//	{
...
//	if ( !Math::IsCloseToZero( plane.m_N.m_Y )
//	{
...
//	if ( !Math::IsCloseToZero( plane.m_N.m_Z )
//	{
//		float const	xx	= plane.m_N.m_X * box.m_X;
//		float const	xxw	= plane.m_N.m_X * ( box.m_X + box.m_W );
...
Intersectable::IntersectionClass Intersectable::Intersects( AABox const & aabox, Frustum const & frustum )
{
	...
	//
	// The box intersects the frustum if it is not in front of any of the planes of the frustum. The box is enclosed by the frustum if none of its vertexes is in front of any of the planes
	// of the frustum.
...
{
	if ( Math::IsCloseToZero( Dot( line.m_M, halfspace.m_Plane.m_N ) ) )
	{
...
int	Intersectable::Intersection( Line const & a, Line const & b, float * ps, float * pt )
{
	...
	if ( Math::IsCloseToZero( n.Length2(), 2.*Math::DEFAULT_FLOAT_TOLERANCE ) )
	{
		...
		{
			return numeric_limits<int>::infinity();	// Co-linear, return infinity
		}
		else
		{
...
int	Intersectable::Intersection( Line const & line, Plane const & plane, float * pt )
{
	...
	if ( Math::IsCloseToZero( dot ) )
	{
		if ( Math::IsCloseToZero( distance ) )
		{
			return numeric_limits<int>::infinity();	// Line is co-planar
		}
		else
		{
...
{
	assert( !Math::IsCloseToZero( segment.m_M.Length2() ) );
...
float Distance( Line const & a, Line const & b )
{
	...
	if ( !Math::IsCloseToZero( cross.Length2(), 2.*Math::DEFAULT_FLOAT_TOLERANCE ) )
	{
