/** @file *//********************************************************************************************************
	...
	$Header: //depot/Libraries/Math/Matrix43.cpp#13 $
...
Matrix43::Matrix43( Matrix33 const & m33, Vector3 const & t/* = Vector3::Origin()*/ )
	: m_Xx( m33.m_Xx ), m_Xy( m33.m_Xy ), m_Xz( m33.m_Xz ),
	  m_Yx( m33.m_Yx ), m_Yy( m33.m_Yy ), m_Yz( m33.m_Yz ),
	  m_Zx( m33.m_Zx ), m_Zy( m33.m_Zy ), m_Zz( m33.m_Zz ),
...
Matrix43::Matrix43( Matrix43d const & m43d )
	: m_Xx( float( m43d.m_Xx ) ), m_Xy( float( m43d.m_Xy ) ), m_Xz( float( m43d.m_Xz ) ),
	  m_Yx( float( m43d.m_Yx ) ), m_Yy( float( m43d.m_Yy ) ), m_Yz( float( m43d.m_Yz ) ),
	  m_Zx( float( m43d.m_Zx ) ), m_Zy( float( m43d.m_Zy ) ), m_Zz( float( m43d.m_Zz ) ),
	  m_Tx( float( m43d.m_Tx ) ), m_Ty( float( m43d.m_Ty ) ), m_Tz( float( m43d.m_Tz ) )
{
}
...
Matrix43::Matrix43( Matrix44 const & m44 )
	: m_Xx( m44.m_Xx ), m_Xy( m44.m_Xy ), m_Xz( m44.m_Xz ),
	  ...
	  m_Tx( m44.m_Tx ), m_Ty( m44.m_Ty ), m_Tz( m44.m_Tz )
{
}
...
Matrix43 & Matrix43::Invert()
{
	...
	assert( !Math::IsCloseToZero( detSR, Math::DEFAULT_DOUBLE_TOLERANCE ) );

	if ( !Math::IsCloseToZero( detSR, Math::DEFAULT_DOUBLE_TOLERANCE ) )
	{
