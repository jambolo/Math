...
Matrix22::Matrix22( Vector2 const & x, Vector2 const & y )
	: m_Xx( x.m_X ), m_Xy( x.m_Y ),
	  m_Yx( y.m_X ), m_Yy( y.m_Y )
{
}
...
Matrix22::Matrix22( Matrix22d const & m22d )
	: m_Xx( float( m22d.m_Xx ) ), m_Xy( float( m22d.m_Xy ) ),
	  m_Yx( float( m22d.m_Yx ) ), m_Yy( float( m22d.m_Yy ) )
{
}
...
Matrix22 & Matrix22::Invert()
{
	...
	assert( !Math::IsCloseToZero( det ) );

	if ( !Math::IsCloseToZero( det ) )
	{
		m_Xx =  a.m_Yy / det;
		m_Xy = -a.m_Xy / det;
