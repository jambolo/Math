...
protected:
	...
	static IntersectionClass Intersects( Line const & line, Point const & point )				{ return ReversedEnclosure( Intersects( point, line ) );		}
	...
	static IntersectionClass Intersects( Ray const & ray, Point const & point )					{ return ReversedEnclosure( Intersects( point, ray ) );			}
																							
	//! Returns the class of intersection between the ray and the line.						
	static IntersectionClass Intersects( Ray const & ray, Line const & line )					{ return ReversedEnclosure( Intersects( line, ray ) );			}
	...
	static IntersectionClass Intersects( Segment const & segment, Point const & point )			{ return ReversedEnclosure( Intersects( point, segment ) );		}
																							
	//! Returns the class of intersection between the segment and the line.					
	static IntersectionClass Intersects( Segment const & segment, Line const & line )			{ return ReversedEnclosure( Intersects( line, segment ) );		}
																							
	//! Returns the class of intersection between the segment and the ray.					
	static IntersectionClass Intersects( Segment const & segment, Ray const & ray )				{ return ReversedEnclosure( Intersects( ray, segment ) );		}
	...
	static IntersectionClass Intersects( Plane const & plane, Point const & point )				{ return ReversedEnclosure( Intersects( point, plane ) );		}
																							
	//! Returns the class of intersection between the plane and the line.					
	static IntersectionClass Intersects( Plane const & plane, Line const & line )				{ return ReversedEnclosure( Intersects( line, plane ) );		}
																							
	//! Returns the class of intersection between the plane and the ray.					
	static IntersectionClass Intersects( Plane const & plane, Ray const & ray )					{ return ReversedEnclosure( Intersects( ray, plane ) );			}
																							
	//! Returns the class of intersection between the plane and the segment.				
	static IntersectionClass Intersects( Plane const & plane, Segment const & segment )			{ return ReversedEnclosure( Intersects( segment, plane ) );		}
	...
	static IntersectionClass Intersects( HalfSpace const & halfspace, Point const & point )		{ return ReversedEnclosure( Intersects( point, halfspace ) );	}

	//! Returns the class of intersection between the halfspace and the line.
	static IntersectionClass Intersects( HalfSpace const & halfspace, Line const & line )		{ return ReversedEnclosure( Intersects( line, halfspace ) );	}

	//! Returns the class of intersection between the halfspace and the ray.
	static IntersectionClass Intersects( HalfSpace const & halfspace, Ray const & ray )			{ return ReversedEnclosure( Intersects( ray, halfspace ) );		}

	//! Returns the class of intersection between the halfspace and the segment.
	static IntersectionClass Intersects( HalfSpace const & halfspace, Segment const & segment )	{ return ReversedEnclosure( Intersects( segment, halfspace ) );	}

	//! Returns the class of intersection between the halfspaces.
	static IntersectionClass Intersects( HalfSpace const & halfspace, Plane const & plane )		{ return ReversedEnclosure( Intersects( plane, halfspace ) );	}
	...
	static IntersectionClass Intersects( Poly const & poly, Point const & point )				{ return ReversedEnclosure( Intersects( point, poly ) );		}
																							
	//! Returns the class of intersection between the poly and the line.					
	static IntersectionClass Intersects( Poly const & poly, Line const & line )					{ return ReversedEnclosure( Intersects( line, poly ) );			}
																							
	//! Returns the class of intersection between the poly and the ray.						
	static IntersectionClass Intersects( Poly const & poly, Ray const & ray )					{ return ReversedEnclosure( Intersects( ray, poly ) );			}
																							
	//! Returns the class of intersection between the poly and the segment.					
	static IntersectionClass Intersects( Poly const & poly, Segment const & segment )			{ return ReversedEnclosure( Intersects( segment, poly ) );		}
																							
	//! Returns the class of intersection between the poly and the plane.					
	static IntersectionClass Intersects( Poly const & poly, Plane const & plane )				{ return ReversedEnclosure( Intersects( plane, poly ) );		}
																							
	//! Returns the class of intersection between the poly and the half-space.				
	static IntersectionClass Intersects( Poly const & poly, HalfSpace const & halfspace )		{ return ReversedEnclosure( Intersects( halfspace, poly ) );	}
	...
	static IntersectionClass Intersects( Sphere const & sphere, Point const & point )			{ return ReversedEnclosure( Intersects( point, sphere ) );		}

	//! Returns the class of intersection between the sphere and the line.
	static IntersectionClass Intersects( Sphere const & sphere, Line const & line )				{ return ReversedEnclosure( Intersects( line, sphere ) );		}

	//! Returns the class of intersection between the sphere and the ray.
	static IntersectionClass Intersects( Sphere const & sphere, Ray const & ray )				{ return ReversedEnclosure( Intersects( ray, sphere ) );		}

	//! Returns the class of intersection between the sphere and the segment.
	static IntersectionClass Intersects( Sphere const & sphere, Segment const & segment )		{ return ReversedEnclosure( Intersects( segment, sphere ) );	}

	//! Returns the class of intersection between the sphere and the plane.
	static IntersectionClass Intersects( Sphere const & sphere, Plane const & plane )			{ return ReversedEnclosure( Intersects( plane, sphere ) );		}

	//! Returns the class of intersection between the sphere and the half-space.
	static IntersectionClass Intersects( Sphere const & sphere, HalfSpace const & halfspace )	{ return ReversedEnclosure( Intersects( halfspace, sphere ) );	}

	//! Returns the class of intersection between the sphere and the poly.
	static IntersectionClass Intersects( Sphere const & sphere, Poly const & poly )				{ return ReversedEnclosure( Intersects( poly, sphere ) );		}
	...
	static IntersectionClass Intersects( Cone const & cone, Point const & point )				{ return ReversedEnclosure( Intersects( point, cone ) );		}
																							
	//! Returns the class of intersection between the cone and the line.					
	static IntersectionClass Intersects( Cone const & cone, Line const & line )					{ return ReversedEnclosure( Intersects( line, cone ) );			}
																							
	//! Returns the class of intersection between the cone and the ray.						
	static IntersectionClass Intersects( Cone const & cone, Ray const & ray )					{ return ReversedEnclosure( Intersects( ray, cone ) );			}
																							
	//! Returns the class of intersection between the cone and the segment.					
	static IntersectionClass Intersects( Cone const & cone, Segment const & segment )			{ return ReversedEnclosure( Intersects( segment, cone ) );		}
																							
	//! Returns the class of intersection between the cone and the plane.					
	static IntersectionClass Intersects( Cone const & cone, Plane const & plane )				{ return ReversedEnclosure( Intersects( plane, cone ) );		}
																							
	//! Returns the class of intersection between the cone and the half-space.				
	static IntersectionClass Intersects( Cone const & cone, HalfSpace const & halfspace )		{ return ReversedEnclosure( Intersects( halfspace, cone ) );	}
																							
	//! Returns the class of intersection between the cone and the poly.					
	static IntersectionClass Intersects( Cone const & cone, Poly const & poly )					{ return ReversedEnclosure( Intersects( poly, cone ) );			}
																							
	//! Returns the class of intersection between the cone and the sphere.					
	static IntersectionClass Intersects( Cone const & cone, Sphere const & sphere )				{ return ReversedEnclosure( Intersects( sphere, cone ) );		}
	...
	static IntersectionClass Intersects( AABox const & aabox, Point const & point )				{ return ReversedEnclosure( Intersects( point, aabox ) );   	}
																							
	//! Returns the class of intersection between the AA box and the line.					
	static IntersectionClass Intersects( AABox const & aabox, Line const & line )				{ return ReversedEnclosure( Intersects( line, aabox ) );   		}
																							
	//! Returns the class of intersection between the AA box and the ray.					
	static IntersectionClass Intersects( AABox const & aabox, Ray const & ray )					{ return ReversedEnclosure( Intersects( ray, aabox ) );   		}
																							
	//! Returns the class of intersection between the AA box and the segment.				
	static IntersectionClass Intersects( AABox const & aabox, Segment const & segment )			{ return ReversedEnclosure( Intersects( segment, aabox ) ); 	}
																							
	//! Returns the class of intersection between the AA box and the plane.					
	static IntersectionClass Intersects( AABox const & aabox, Plane const & plane )				{ return ReversedEnclosure( Intersects( plane, aabox ) );   	}
																							
	//! Returns the class of intersection between the AA box and the half-space.			
	static IntersectionClass Intersects( AABox const & aabox, HalfSpace const & halfspace )		{ return ReversedEnclosure( Intersects( halfspace, aabox ) );	}
																							
	//! Returns the class of intersection between the AA box and the poly.					
	static IntersectionClass Intersects( AABox const & aabox, Poly const & poly )				{ return ReversedEnclosure( Intersects( poly, aabox ) );   		}
																							
	//! Returns the class of intersection between the AA box and the sphere.				
	static IntersectionClass Intersects( AABox const & aabox, Sphere const & sphere )			{ return ReversedEnclosure( Intersects( sphere, aabox ) );  	}
																							
	//! Returns the class of intersection between the AA box and the cone.					
	static IntersectionClass Intersects( AABox const & aabox, Cone const & cone )				{ return ReversedEnclosure( Intersects( cone, aabox ) );		}
	...
	static IntersectionClass Intersects( Box const & box, Point const & point )					{ return ReversedEnclosure( Intersects( point, box ) );  		}
																							
	//! Returns the class of intersection between the box and the line.						
	static IntersectionClass Intersects( Box const & box, Line const & line )					{ return ReversedEnclosure( Intersects( line, box ) );			}
																							
	//! Returns the class of intersection between the box and the ray.						
	static IntersectionClass Intersects( Box const & box, Ray const & ray )						{ return ReversedEnclosure( Intersects( ray, box ) );	 		}
																							
	//! Returns the class of intersection between the box and the segment.					
	static IntersectionClass Intersects( Box const & box, Segment const & segment )				{ return ReversedEnclosure( Intersects( segment, box ) );  		}
																							
	//! Returns the class of intersection between the box and the plane.					
	static IntersectionClass Intersects( Box const & box, Plane const & plane )					{ return ReversedEnclosure( Intersects( plane, box ) );  		}
																							
	//! Returns the class of intersection between the box and the half-space.				
	static IntersectionClass Intersects( Box const & box, HalfSpace const & halfspace )			{ return ReversedEnclosure( Intersects( halfspace, box ) );		}
																							
	//! Returns the class of intersection between the box and the poly.						
	static IntersectionClass Intersects( Box const & box, Poly const & poly )					{ return ReversedEnclosure( Intersects( poly, box ) );  		}
																							
	//! Returns the class of intersection between the box and the sphere.					
	static IntersectionClass Intersects( Box const & box, Sphere const & sphere )				{ return ReversedEnclosure( Intersects( sphere, box ) ); 		}
																							
	//! Returns the class of intersection between the box and the cone.						
	static IntersectionClass Intersects( Box const & box, Cone const & cone )					{ return ReversedEnclosure( Intersects( cone, box ) );			}
																							
	//! Returns the class of intersection between the box and the aabox.					
	static IntersectionClass Intersects( Box const & box, AABox const & aabox )					{ return ReversedEnclosure( Intersects( aabox, box ) );     	}
	...
	static IntersectionClass Intersects( Frustum const & frustum, Point const & point )			{ return ReversedEnclosure( Intersects( point, frustum ) ); 	}

	//! Returns the class of intersection between the frustum and the line.
	static IntersectionClass Intersects( Frustum const & frustum, Line const & line )			{ return ReversedEnclosure( Intersects( line, frustum ) ); 		}

	//! Returns the class of intersection between the frustum and the ray.
	static IntersectionClass Intersects( Frustum const & frustum, Ray const & ray )				{ return ReversedEnclosure( Intersects( ray, frustum ) ); 		}

	//! Returns the class of intersection between the frustum and the segment.
	static IntersectionClass Intersects( Frustum const & frustum, Segment const & segment )		{ return ReversedEnclosure( Intersects( segment, frustum ) );	}

	//! Returns the class of intersection between the frustum and the plane.
	static IntersectionClass Intersects( Frustum const & frustum, Plane const & plane )			{ return ReversedEnclosure( Intersects( plane, frustum ) ); 	}

	//! Returns the class of intersection between the frustum and the half-space.
	static IntersectionClass Intersects( Frustum const & frustum, HalfSpace const & halfspace )	{ return ReversedEnclosure( Intersects( halfspace, frustum ) );	}

	//! Returns the class of intersection between the frustum and the poly.
	static IntersectionClass Intersects( Frustum const & frustum, Poly const & poly )			{ return ReversedEnclosure( Intersects( poly, frustum ) ); 		}

	//! Returns the class of intersection between the frustum and the sphere.
	static IntersectionClass Intersects( Frustum const & frustum, Sphere const & sphere )		{ return ReversedEnclosure( Intersects( sphere, frustum ) );	}

	//! Returns the class of intersection between the frustum and the cone.
	static IntersectionClass Intersects( Frustum const & frustum, Cone const & cone )			{ return ReversedEnclosure( Intersects( cone, frustum ) );		}

	//! Returns the class of intersection between the frustum and the aabox.
	static IntersectionClass Intersects( Frustum const & frustum, AABox const & aabox )			{ return ReversedEnclosure( Intersects( aabox, frustum ) ); 	}

	//! Returns the class of intersection between the frustum and the box.
	static IntersectionClass Intersects( Frustum const & frustum, Box const & box )				{ return ReversedEnclosure( Intersects( box, frustum ) );   	}
