/** @file *//********************************************************************************************************
	...
	$Header: //depot/Libraries/Math/Quaternion.cpp#14 $
...
void Quaternion::GetRotationAxisAndAngle( Vector3 * pAxis, float * pAngle ) const
{
	...
	float const	w	= limit( -1.0f, m_W, 1.0f );	// Limit w so that acos() and sqrt() are happy
	float const	s	= sqrtf( 1.f - w * w );			// = sin( .5 * 2. * acos( m_W ) );
													// also = sqrt( m_X * m_X + m_Y * m_Y + m_Z * m_Z )
	if ( !Math::IsCloseToZero( s ) )
	{
		pAxis->m_X = m_X / s;
		pAxis->m_Y = m_Y / s;
		pAxis->m_Z = m_Z / s;
